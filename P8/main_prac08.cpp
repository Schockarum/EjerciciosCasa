...
void myData() //Recordemos que antes aquí teníamos un cubo, pero ahora hay un plano.
{
	float vertices[] = {  //La posicion es el vértice que se dibuja y la coordenada de la textura es la que le corresponde a ese vértice.
		...
		//frontal
		0.4f, 0.5f, 0.4f,		1.0f, 1.0f, 	// top right 
		0.4f, -0.7f, 0.4f,		1.0f, 0.0f, 	// bottom right
		-0.4f, -0.7f, 0.4f,		0.0f, 0.0f, 	// bottom left
		-0.4f, 0.5f, 0.4f,		0.0f, 1.0f, 	// top left
		...
		//superior
		2.0f, 0.5f, -2.0f,		1.0f, 1.0f,		// top right 
		2.0f, 0.5f, 2.0f,		1.0f, 0.0f, 	// bottom right
		-2.0f, 0.5f, 2.0f,		0.0f, 0.0f, 	// bottom left
		-2.0f, 0.5f, -2.0f,		0.0f, 1.0f, 	// top left
		...
		//inferior
		0.4f, -0.7f, -0.4f,		1.0f, 1.0f,		// top right
		0.4f, -0.7f, 0.4f,		1.0f, 0.0f, 	// bottom right
		-0.4f, -0.7f, 0.4f,		0.0f, 0.0f, 	// bottom left
		-0.4f, -0.7f, -0.4f,	0.0f, 1.0f, 	// top left

		//tablero
		1.5f, 0.51f, -1.5f,		4.0f, 0.0f, //der aba
		1.5f, 0.51f, 1.5f,		4.0f, 4.0f, //der arr
		-1.5f, 0.51f, 1.5f,		0.0f, 4.0f, //izq arr
		-1.5f, 0.51f, -1.5f,	0.0f, 0.0f, //izq aba
	};
	...
	unsigned char *data = stbi_load("Texturas/bricks2.jpg", &width, &height, &nrChannels, 0); //Direccion de la textura en carpeta del proyecto.
	if (data)
	{
		// IMPORTANT note
...
void display(void)
{
	...
	glActiveTexture(GL_TEXTURE);//Lo primero que debe de hacerse es esto, activar la textura a 0. Es como la localidad de memoria de la textura
	...
	glBindVertexArray(VAO);
	glDrawElements(GL_TRIANGLES, 36, GL_UNSIGNED_INT, 0); //Dibujamos la mesa

	glActiveTexture(GL_TEXTURE);//creamos la segunda textura
	glBindTexture(GL_TEXTURE_2D, texture2);//atamos la textura
	projectionShader.setInt("texture2", 1);
	
	glDrawArrays(GL_TRIANGLE_FAN, 24, 4); //Dibujamos el tablero
